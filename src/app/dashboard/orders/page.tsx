...
import { Bike, Eye, Loader2, PackageOpen, ThumbsUp } from "lucide-react";
                                        ...
                                        <div className="flex items-center relative flex-wrap p-2 w-auto shadow justify-between border-b pb-2" key={order._id}>
                                            ...
                                            <div className="flex flex-col h-full gap-1 justify-center items-center">
                                                ...
                                                <button
                                                    ...
                                                    className={`text-sm ${order.status === "pending" ? "bg-blue-500" : order.status === "delivered" ? "bg-green-300" : "bg-green-500"} text-white flex justify-center items-center gap-2 px-2 flex-1 w-24 rounded-bl-lg`}
                                                    onClick={(e) => {
                                                ...
                                                >
                                                    {loadingOrder === order._id ? (
                                                        ...
                                                        <>
                                                            {order.status === "pending" ? <Bike className="size-4" /> : order.status === "delivered" ? <ThumbsUp /> : <PackageOpen className="size-4" />}
                                                            <span>{order.status === "pending" ? "Dispatch" : order.status === "delivered" ? "Delivered" : "Deliver"}</span>
                                                        </>
                                                    )}
                                                </button>
                                            </div>
